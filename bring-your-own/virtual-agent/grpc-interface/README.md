# Bring Your Own Virtual Agent — gRPC

The Bring-Your-Own-Virtual-Agent (BYoVA) initiative empowers developers and AI vendors to seamlessly integrate external conversational interfaces with Webex Contact Center (WxCC). The **gRPC** flavour of the integration uses gRPC bidirectional streaming, where each logical interaction (welcome prompt, DTMF input, audio utterance, etc.) is carried on its own short-lived RPC. WxCC opens a fresh RPC for each turn of the conversation and closes it once the turn completes; the conversation as a whole is identified by a stable `conversation_id` that is reused across every RPC of the same call.
...
## gRPC Virtual Agent Simulators

Reference servers live under [`simulators/`](./simulators/). Each one is a self-contained Spring Boot / Python application that implements exactly the same `VoiceVirtualAgent` gRPC contract (`ProcessCallerInput` bidirectional stream + `ListVirtualAgents` unary RPC), so they are interchangeable from the WxCC client's point of view — pick whichever language is closer to your stack.
...
1. Your Virtual Agent service starts up as a gRPC Virtual Agent Server (**VA Server**).
...
For partial responses, `VoiceVAResponse.disable_prompt_cancellation` can be set to `true` to prevent the current partial prompt from being canceled by a later VA response; it defaults to `false` and does not change caller barge-in behavior.
