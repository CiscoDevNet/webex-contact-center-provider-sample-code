...
current_dir = os.path.dirname(os.path.abspath(__file__))  # This is 'server' directory
parent_dir = os.path.dirname(current_dir)  # This is 'src' directory
...
PORT = int(os.environ.get('GRPC_SERVER_PORT', 8086))
...
class AIAgent(voicevirtualagent_pb2_grpc.VoiceVirtualAgentServicer):
    ...
    def ListVirtualAgents(self, request, context):
        try:
            ...
            print("Returning List VA response")
            return response
    ...
    def ProcessCallerInput(self, request_iterator, context):
        ...
        try:
            ...
            if conversation_id and conversation_id in self.state:
                self.state.pop(conversation_id, None)
